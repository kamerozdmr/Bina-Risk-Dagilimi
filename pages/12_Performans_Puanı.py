...
try:
    if st.session_state["bina_tipi"] == "Betonarme":
        ...
        parametre_liste = ["Bina Tipi", "Sistem Tipi", "Enlem", "Boylam", "Adres Bilgisi", "Spektral ivme katsayısını (Sds)", "Zemin Sınıfı","Kat Adedi", "Yapım Yılı", "Görsel Kalite", "Düşeyde Düzensizlik", "Yumuşak/Zayıf Kat", "Ağır Çıkma", "Kısa Kolon", "Zemin Eğimi", 
                        "Planda Düzensizlik", "Nizam Durumu", "Bitişik Yapı Konumu", "Bitişik Yapı Döşeme Seviyesi", "Diğer Bilgiler"]
        parametre_tespit_liste = [bina_tipi, sistem_tipi, enlem, boylam, adres, ivme_katsayisi, zemin_sinifi, kat_adedi, yapim_yili, SliderDegeri(gorsel_kalite), duseyde_duzensizlik,
                                yumusak_kat, agir_cikma, kisa_kolon, zemin_egimi, planda_duzensizlik, nizam_durumu, nizam_konumu, doseme_seviyesi,diger_bilgiler]
        ...
        st.dataframe(parametre_df, width=400, height=740)
        ...
        if st.session_state["enlem"] and st.session_state["boylam"] != "":
            d = {"lat": [st.session_state["enlem"]], "lon": [st.session_state["boylam"]]}
            coord_df = pd.DataFrame(d)

        st.map(coord_df, zoom=13)   #, use_container_width = 720
        ...
        def BetonarmeOlumsuzlukParametrePuanlarınıHesapla(kat_adedi, gorsel_kalite, duseyde_duzensizlik,yumusak_kat, agir_cikma, kisa_kolon, 
            ...
            """ 
            Girdiler : Kat adedi, malzeme türü ve kalitesi, düşeyde düzensizlik, yumuşak kat, ağır çıkma, kısa kolon,
                    zemin etkisi, planda düzensizlik, nizam durumu, bitişik yapının konumu, döşeme seviyesi
            Çıktı : Sıra ile olumsuzluk parametre puanlarını içeren liste
            """
