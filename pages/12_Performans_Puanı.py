...
try:
    if st.session_state["bina_tipi"] == "Betonarme":
        ...
        with col1:
            ...
            st.write(f"Olumsuzluk Puanları Toplamı : {sum(OP)}")
        with col2:
        ...
        parametre_liste = ["Bina Tipi", "Sistem Tipi", "Enlem", "Boylam", "Adres Bilgisi", "Spektral ivme katsayısını (Sds)", "Zemin Sınıfı","Kat Adedi", "Yapım Yılı", "Görsel Kalite", "Düşeyde Düzensizlik", "Yumuşak/Zayıf Kat", "Ağır Çıkma", "Kısa Kolon", "Zemin Eğimi", 
                        "Planda Düzensizlik", "Nizam Durumu", "Bitişik Yapı Konumu", "Bitişik Yapı Döşeme Seviyesi", "Diğer Bilgiler", "Tarih", "Saat"]
        parametre_tespit_liste = [bina_tipi, sistem_tipi, enlem, boylam, adres, ivme_katsayisi, zemin_sinifi, kat_adedi, yapim_yili, SliderDegeri(gorsel_kalite), duseyde_duzensizlik,
                                yumusak_kat, agir_cikma, kisa_kolon, zemin_egimi, planda_duzensizlik, nizam_durumu, nizam_konumu, doseme_seviyesi,diger_bilgiler, tarih_string, saat_string]
        ...
        st.dataframe(parametre_df, width = 480, height = 808)       
        ...
        parametre_liste = ["Bina Tipi", "Sistem Tipi", "Enlem", "Boylam", "Adres Bilgisi", "Spektral ivme katsayısını (Sds)", "Zemin Sınıfı","Kat Adedi", "Yapım Yılı", 
                           ...
                           "Duvar Miktarı", "Nizam Durumu", "Bitişik Yapı Konumu", "Bitişik Yapı Döşeme Seviyesi", "Diğer Bilgiler", "Tarih", "Saat"]

        parametre_tespit_liste = [bina_tipi, sistem_tipi, enlem, boylam, adres, ivme_katsayisi, zemin_sinifi, kat_adedi, yapim_yili, SliderDegeri(malzeme_kalite), SliderDegeri(duvar_isciligi),
                                  ...
                                  harc_malzemesi, SliderDegeriDuzen(planda_duzensizlik), yatay_hatil, SliderDegeri(duvar_miktari), nizam_durumu, nizam_konumu, doseme_seviyesi, diger_bilgiler,
                                  tarih_string, saat_string]

        parametre_df = pd.DataFrame({"Parametreler":parametre_liste, "Parametre Değerleri":parametre_tespit_liste})
        parametre_df = parametre_df.set_index("Parametreler")
        st.dataframe(parametre_df, width = 480, height = 1052)       
