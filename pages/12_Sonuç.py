    ...
    try:
        if st.session_state["bina_tipi"] == "Betonarme":
            ...
            with col1:
                ...
                st.write("---")
                st.write("Aşağıdaki bölümden parametrelerin doğruluğunu kontrol ediniz ve veritabanına kaydediniz.")
            ...
            zaman =  int(time.time()) 
            st.subheader("Girilen Parametreler")
            ...
            st.dataframe(parametre_df, width = 720, height = 950)       
            ...
            if data_submit:
                ...
                EndConnection()
                placeholder.empty()
                st.success("Veritabanına kaydedildi. \n\nYeni bina eklemek için Ana Sayfaya dönüp sayfayı yenileyin.")
            ...
            parametre_df = pd.DataFrame({"Parametreler":parametre_liste, "Parametre Değerleri":parametre_tespit_liste})
            parametre_df = parametre_df.set_index("Parametreler")
            st.dataframe(parametre_df, width = 720, height = 1200)       
